...
class ArboristNode {
  constructor (tree, path) {
    ...
    if (tree.overrides) {
      this.overrides = new Map([...tree.overrides.ruleset.entries()]
        .map(([name, override]) => [override.key, { value: override.value, keySpec: override.keySpec }]))
    }
...
class EdgeOut extends Edge {
  ...
  [util.inspect.custom] () {
    return `{ ${this.type} ${this.name}@${this.spec}${
      ...
      this.overrides ? ` overrode:${this.overrides.keySpec || '*'}->${this.overrides.value}` : ''
    } }`
  }
}
