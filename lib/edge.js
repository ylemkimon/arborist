...
class Edge {
  ...
  satisfiedBy (node) {
    ...
    const rule = this.overrides.getNodeRule(node)
    // getNodeRule returns the root rule if there is no specific match
    if (rule === this.overrides) {
      return depValid(node, this.spec, this.accept, this.from)
    }

    return depValid(node, rule.value, this.accept, this.from)
  }
